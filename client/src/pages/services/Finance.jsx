  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-green-500 to-emerald-600 text-white py-16 px-6 text-center">
        ...
        <p className="text-green-100 text-lg max-w-xl mx-auto">Loans, investments & smart financial planning for your future</p>
      </div>
      <div className="max-w-6xl mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div>
          <h2 className="text-2xl font-black text-gray-800 mb-6">Our Finance Services</h2>
          <div className="space-y-4">
            {[
              { icon: 'fa-house', title: 'Home Loans', desc: 'Best rates for home loans' },
              { icon: 'fa-car', title: 'Vehicle Loans', desc: 'Two-wheeler & four-wheeler loans' },
              { icon: 'fa-briefcase', title: 'Business Loans', desc: 'Grow your business with us' },
              { icon: 'fa-graduation-cap', title: 'Education Loans', desc: 'Invest in your future' },
            ].map(item => (
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          {submitted ? (
            <div className="text-center py-10">
              ...
              <a href={`https://wa.me/919966331389?text=Hi, I need Finance Services. Name: ${form.name}, Loan: ${form.loanType}`}
            ...
            <>
              <h2 className="text-xl font-black text-gray-800 mb-6">Apply for a Loan</h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="text-sm font-semibold text-gray-700 mb-1 block">Loan Amount</label>
                  <input type="text" name="amount" value={form.amount} onChange={handleChange} required placeholder="e.g. ₹5,00,000"
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-400"/>
                </div>
                <div>
                  <label className="text-sm font-semibold text-gray-700 mb-1 block">Message</label>
                  <textarea name="message" value={form.message} onChange={handleChange} rows={3} placeholder="Any additional details..."
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-400"/>
                </div>
                <button type="submit" disabled={loading}
                  className="w-full bg-gradient-to-r from-green-500 to-emerald-500 text-white font-bold py-3 rounded-xl hover:opacity-90 transition hover:scale-105 disabled:opacity-50">
                  {loading ? <><i className="fa-solid fa-spinner fa-spin mr-2"></i>Submitting...</> : <><i className="fa-solid fa-paper-plane mr-2"></i>Submit Application</>}
                </button>
